...
int main(int argc, char **argv) {
    ...
    cv::Point origin;
    cv::Mat kernel;
    cv::Vec3b regionPixelOut(255, 255, 255);
    Utils::loadConfiguration(input_config_path, origin, kernel, regionPixelOut, false);
    ...
    // Generate region
    std::pair<cv::Vec3b, cv::Vec3b> neighbor_min_max = Utils::generateRegion(input, kernel, origin, regionPixelOut, output);
    ...
    double min_magnitude = cv::norm(neighbor_min_max.first, cv::NORM_L2);

    //    namedWindow("Output Image", cv::WINDOW_AUTOSIZE);
    //    imshow("Output Image", output);
    //    cv::waitKey(3000);

    int result = (origin_magnitude < min_magnitude) ? 1 : 0;
